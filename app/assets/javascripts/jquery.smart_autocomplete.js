...
(function($){
  $.fn.smartAutoComplete = function(){    
    ...
    var default_options = {
                            ...
                            typeAhead: false,
                            resultElement: "li",
                            resultFormatter: function(r){ return ("<li class='token-input-token' id='"+ r['id'] +"'>" + r['name'] + "</li>"); },
    ...
    $.event.special.keyIn = {
      ...
      _default: function(ev){
        ...
        var minChars = (options.minCharLimit > 0 ?  options.minCharLimit : Number.POSITIVE_INFINITY)

        //event specific data
        var query = ev.smartAutocompleteData.query;

        if(options.disabled || (query.length > maxChars) || (query.length < minChars)){
          return false;
        }
    ...
    $.event.special.itemSelect = { 
      ...
      _default: function(ev){    
        ...
        //FC01 get id from selected item and set hidden value
        var selected_id = $(selected_item).attr('id');
        var valueInput = $(context).next();
        $(valueInput).val(selected_id);
    ...
    return this.each(function(i) { 
      //set the options
      //FC01 var options = $.extend(default_options, $(this).data("smart-autocomplete"), passed_options);
      //set the context
      //FC01 options['context'] = this;

      //FC01 $(this).data("smart-autocomplete", options);
      ...
      var input_box = $("<input type=\"text\"  autocomplete=\"off\">")
        .attr("id", "lookup_" + this.id)
        .val(hidden_input.data("pre"))
        .insertBefore(hidden_input);

      $(input_box).data("smart-autocomplete", options);
      options['context'] = input_box;
      ...
      if($.type(options.resultsContainer) === 'undefined' ){
        ...
        var default_container = $("<ul class='token-input-list' style='display:none'></ul>")
            .mouseover(function (event) {
              $(options.context).trigger('itemFocus', [ $(event.target).closest("li") ] );
            })
            .mousedown(function (event) {
              $(options.context).trigger('itemSelect', [ $(event.target).closest("li") ] );
              return false;
            })
            .mouseout(function (event) {
              $(options.context).trigger('itemUnfocus', [ $(event.target).closest("li") ] );
            });
      ...
      // bind user events
      //FC01 $(this).keyup(function(ev){
      $(input_box).keydown(function(ev){
        //get the options
        //FC01 var options = $(this).data("smart-autocomplete");
        var options = $(input_box).data("smart-autocomplete");
        ...
        //right arrow & enter key
         ...
         if(current_char_count >= options.minCharLimit){
          //setTimeout($(options.context).trigger('keyIn', [$(this).val()]), 3000);
          $(options.context).trigger('keyIn', [$(this).val()]); 
         }
         else{
            if(options.autocompleteFocused){ 
      ...
      $(input_box).focus(function(){
        ...
        $(input_box).closest("form").bind("keydown.block_for_smart_autocomplete", function(ev){
          var type_ahead_field = $(options.context).prev('.smart_autocomplete_type_ahead_field');
          if(ev.keyCode == '13'){
            if(options.resultsContainer && $(options.resultsContainer).is(':visible')){
              var current_selection = options.currentSelection;
